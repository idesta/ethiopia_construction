...
const MAIL_FROM = process.env.MAIL_FROM || "noreply@ethioconstruction.abrdns.com";
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const result = await client_1.db.query(`SELECT t.name, t.slug, c.email
       FROM tenants t
       LEFT JOIN contacts c ON c.tenant_id = t.id
       WHERE t.slug = $1 AND t.is_active = TRUE`, [slug]);
    ...
    `;
        ...
        if (!resend) {
            ...
            res.json({
                success: true,
                message: "Message received. Email service is not configured, so no email was sent.",
            });
            return;
        }
        try {
